namespace SpaceEquipmentLtd.NanobotDrillSystem
{
  ...
  public static class Texts
  {
    ...
    static Texts()
    {
      MyLanguagesEnum language = Mod.DisableLocalization ? MyLanguagesEnum.English : MyAPIGateway.Session.Config.Language;
      Mod.Log.Write(Logging.Level.Error, "Localization: Disabled={0} Language={1}", Mod.DisableLocalization, language);

      Dictionary<string, string> texts = LocalizationHelper.GetTexts(language, GetDictionaries(), Mod.Log);
    ...
    {
      Dictionary<MyLanguagesEnum, Dictionary<string, string>> dicts = new Dictionary<MyLanguagesEnum, Dictionary<string, string>>
         {
