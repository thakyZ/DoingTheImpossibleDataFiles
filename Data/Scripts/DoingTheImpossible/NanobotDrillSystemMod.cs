namespace SpaceEquipmentLtd.NanobotDrillSystem
{
  ...
  */
  ...
  {
    private const string Version = "V1.0.9 2020-04-02"
    ;
    ...
    public void Init()
    {
      ...
      else
      {
        ...
        MyAPIGateway.Multiplayer.RegisterMessageHandler(MSGID_BLOCK_VOXELBOXES_FILL_REMOVE_FROM_SERVER, SyncBlockVoxelBoxesFillRemoveReceived);
      ...
      Mod.Log.Write("DrillSystemMod: Initialized.");
    }
    ...
    {
      foreach (KeyValuePair<long, NanobotDrillSystemBlock> entry in DrillSystems)
      {
    ...
    public static void InitControls()
    {
      ...
      if (SettingsValid && !NanobotDrillSystemTerminal.CustomControlsInit && DrillSystems.Count > 0)
      {
        NanobotDrillSystemTerminal.InitializeControls();
      }
    }
    ...
    private void Utilities_MessageEntered(string messageText, ref bool sendToOthers)
    {
      ...
      string cmd = messageText.ToLower();
      if (cmd.StartsWith(CmdKey))
      {
        if (Mod.Log.ShouldLog(Logging.Level.Verbose))
        {
          Mod.Log.Write(Logging.Level.Verbose, "DrillSystemMod: Cmd: {0}", messageText);
        }

        string[] args = cmd.Remove(0, CmdKey.Length).Trim().Split(' ');
        if (args.Length > 0)
        {
          if (Mod.Log.ShouldLog(Logging.Level.Verbose))
          {
            Mod.Log.Write(Logging.Level.Verbose, "DrillSystemMod: Cmd args[0]: {0}", args[0]);
          }
    ...
    {
      string text = string.Format(Texts.Cmd_HelpClient.String, Version, CmdHelp1, CmdHelp2, CmdLogLevel, CmdLogLevel_All, CmdLogLevel_Default);
      if (MyAPIGateway.Session.IsServer)
      {
    ...
    protected override void UnloadData()
    {
      while (true)
      {
        ...
        if (actualBackgroundTaskCount <= 0)
        {
          break;
        }
      }
      ...
      try
      {
        if (MyAPIGateway.Utilities != null && MyAPIGateway.Multiplayer != null)
        {
          ...
          else
          {
            ...
            MyAPIGateway.Multiplayer.UnregisterMessageHandler(MSGID_BLOCK_VOXELBOXES_FILL_REMOVE_FROM_SERVER, SyncBlockVoxelBoxesFillRemoveReceived);
            phase = 4;
          }
        }
        phase = 5;
      ...
      {
        Mod.Log.Error("NanobotDrillSystemMod.UnloadData: Phase{0}, {1}", phase, e.ToString());
      }
      base.UnloadData();
    }
    ...
    public void BeforeDamageHandlerNoDamageByDrillSystem(object target, ref MyDamageInformation info)
    {
      try
      {
        if (info.Type == MyDamageType.Drill)
        {
          ...
          {
            NanobotDrillSystemBlock logicalComponent = DrillSystems.GetValueOrDefault(info.AttackerId);
            if (logicalComponent != null)
            {
              IMyTerminalBlock terminalBlock = logicalComponent.Entity as IMyTerminalBlock;
              if (Mod.Log.ShouldLog(Logging.Level.Communication))
              {
                Mod.Log.Write(Logging.Level.Communication, "DrillSystemMod: Prevent Damage from DrillSystem={0} Amount={1}", terminalBlock != null ? terminalBlock.CustomName : logicalComponent.Entity.DisplayName, info.Amount);
              }
    ...
    protected void RebuildSourcesAndTargetsTimer()
    {
      ...
      {
        foreach (NanobotDrillSystemBlock DrillSystem in DrillSystems.Values)
        {
    ...
    public static void AddAsyncAction(Action newAction)
    {
      lock (AsynActions)
      {
        ...
        if (ActualBackgroundTaskCount < Settings.MaxBackgroundTasks)
        {
          ...
          {
            Mod.Log.Write(Logging.Level.Info, "DrillSystemMod: AddAsyncAction Start Task {0} of max {1}", ActualBackgroundTaskCount, Settings.MaxBackgroundTasks);
          }
          ...
          MyAPIGateway.Parallel.StartBackground(() =>
          {
            try
            {
              while (true)
              {
                ...
                if (pendingAction != null)
                {
                  try
                  {
                    if (Mod.Log.ShouldLog(Logging.Level.Info))
                    {
                      Mod.Log.Write(Logging.Level.Info, "DrillSystemMod: AddAsyncAction Task Working start.");
                    }

                    pendingAction();
                    if (Mod.Log.ShouldLog(Logging.Level.Info))
                    {
                      Mod.Log.Write(Logging.Level.Info, "DrillSystemMod: AddAsyncAction Task Working finished.");
                    }
                  }
                  catch { };
                }
              ...
              if (Mod.Log.ShouldLog(Logging.Level.Info))
              {
                Mod.Log.Write(Logging.Level.Info, "DrillSystemMod: AddAsyncAction Task ended. Still running {0} of max {1}", ActualBackgroundTaskCount, Settings.MaxBackgroundTasks);
              }
            }
            catch
            {
    ...
    private void SyncModDataRequestSend()
    {
      ...
      MsgModDataRequest msgSnd = new MsgModDataRequest();
      ...
      if (Mod.Log.ShouldLog(Logging.Level.Communication))
      {
        Mod.Log.Write(Logging.Level.Communication, "DrillSystemMod: SyncModDataRequestSend SteamId={0}", msgSnd.SteamId);
      }
    ...
    {
      MsgModDataRequest msgRcv = MyAPIGateway.Utilities.SerializeFromBinary<MsgModDataRequest>(dataRcv);
      if (Mod.Log.ShouldLog(Logging.Level.Communication))
      {
        Mod.Log.Write(Logging.Level.Communication, "DrillSystemMod: SyncModDataRequestReceived SteamId={0}", msgRcv.SteamId);
      }
    ...
    private void SyncModSettingsSend(ulong steamId)
    {
      ...
      MsgModSettings msgSnd = new MsgModSettings
      {
        Settings = Settings
      };
      if (Mod.Log.ShouldLog(Logging.Level.Communication))
      {
        Mod.Log.Write(Logging.Level.Communication, "DrillSystemMod: SyncModSettingsSend SteamId={0}", steamId);
      }
      ...
      if (!MyAPIGateway.Multiplayer.SendMessageTo(MSGID_MOD_SETTINGS, MyAPIGateway.Utilities.SerializeToBinary(msgSnd), steamId, true))
      {
        ...
        {
          Mod.Log.Write(Logging.Level.Error, "DrillSystemMod: SyncModSettingsSend failed");
        }
      }
    }
    ...
    private void SyncModSettingsReceived(byte[] dataRcv)
    {
      try
      {
        MsgModSettings msgRcv = MyAPIGateway.Utilities.SerializeFromBinary<MsgModSettings>(dataRcv);
        Settings = msgRcv.Settings;
        SettingsValid = true;
        if (Mod.Log.ShouldLog(Logging.Level.Communication))
        {
          Mod.Log.Write(Logging.Level.Communication, "DrillSystemMod: SyncModSettingsReceived");
        }
    ...
    public static void SyncBlockDataRequestSend(NanobotDrillSystemBlock block)
    {
      ...
      MsgBlockDataRequest msgSnd = new MsgBlockDataRequest();
      if (MyAPIGateway.Session.Player != null)
      {
        msgSnd.SteamId = MyAPIGateway.Session.Player.SteamUserId;
      }
      ...
      if (Mod.Log.ShouldLog(Logging.Level.Communication))
      {
        Mod.Log.Write(Logging.Level.Communication, "DrillSystemMod: SyncBlockDataRequestSend SteamId={0} EntityId={1}/{2}", msgSnd.SteamId, msgSnd.EntityId, Logging.BlockName(block.Entity, Logging.BlockNameOptions.None));
      }
    ...
    private void SyncBlockDataRequestReceived(byte[] dataRcv)
    {
      ...
      MsgBlockDataRequest msgRcv = MyAPIGateway.Utilities.SerializeFromBinary<MsgBlockDataRequest>(dataRcv);
      ...
      if (DrillSystems.TryGetValue(msgRcv.EntityId, out system))
      {
        if (Mod.Log.ShouldLog(Logging.Level.Communication))
        {
          Mod.Log.Write(Logging.Level.Communication, "DrillSystemMod: SyncBlockDataRequestReceived SteamId={0} EntityId={1}/{2}", msgRcv.SteamId, msgRcv.EntityId, Logging.BlockName(system.Entity, Logging.BlockNameOptions.None));
        }
      ...
      else
      {
        if (Mod.Log.ShouldLog(Logging.Level.Error))
        {
          Mod.Log.Write(Logging.Level.Error, "DrillSystemMod: SyncBlockDataRequestReceived for unknown system SteamId{0} EntityId={1}", msgRcv.SteamId, msgRcv.EntityId);
        }
      }
    }
    ...
    public static void SyncBlockSettingsSend(ulong steamId, NanobotDrillSystemBlock block)
    {
      ...
      MsgBlockSettings msgSnd = new MsgBlockSettings
      {
        EntityId = block.Entity.EntityId,
        Settings = block.Settings.GetTransmit()
      };
      ...
      if (MyAPIGateway.Session.IsServer)
      {
        if (steamId == 0)
        {
          if (Mod.Log.ShouldLog(Logging.Level.Communication))
          {
            Mod.Log.Write(Logging.Level.Communication, "DrillSystemMod: SyncBlockSettingsSend To Others EntityId={0}/{1}", block.Entity.EntityId, Logging.BlockName(block.Entity, Logging.BlockNameOptions.None));
          }
        ...
        else
        {
          if (Mod.Log.ShouldLog(Logging.Level.Communication))
          {
            Mod.Log.Write(Logging.Level.Communication, "DrillSystemMod: SyncBlockSettingsSend To SteamId={2} EntityId={0}/{1}", block.Entity.EntityId, Logging.BlockName(block.Entity, Logging.BlockNameOptions.None), steamId);
          }
      ...
      else
      {
        if (Mod.Log.ShouldLog(Logging.Level.Communication))
        {
          Mod.Log.Write(Logging.Level.Communication, "DrillSystemMod: SyncBlockSettingsSend To Server EntityId={0}/{1} to Server", block.Entity.EntityId, Logging.BlockName(block.Entity, Logging.BlockNameOptions.None));
        }
      ...
      {
        Mod.Log.Write(Logging.Level.Error, "DrillSystemMod: SyncBlockSettingsSend failed", Logging.BlockName(block.Entity, Logging.BlockNameOptions.None));
      }
    }
    ...
    private void SyncBlockSettingsReceived(byte[] dataRcv)
    {
      try
      {
        MsgBlockSettings msgRcv = MyAPIGateway.Utilities.SerializeFromBinary<MsgBlockSettings>(dataRcv);
        ...
        if (DrillSystems.TryGetValue(msgRcv.EntityId, out system))
        {
          if (Mod.Log.ShouldLog(Logging.Level.Communication))
          {
            Mod.Log.Write(Logging.Level.Communication, "DrillSystemMod: SyncBlockSettingsReceived EntityId={0}/{1}", msgRcv.EntityId, Logging.BlockName(system.Entity, Logging.BlockNameOptions.None));
          }
        ...
        else
        {
          if (Mod.Log.ShouldLog(Logging.Level.Error))
          {
            Mod.Log.Write(Logging.Level.Error, "DrillSystemMod: SyncBlockSettingsReceived for unknown system EntityId={0}", msgRcv.EntityId);
          }
        }
      }
      catch (Exception ex)
    ...
    public static void SyncBlockStateSend(ulong steamId, NanobotDrillSystemBlock system)
    {
      ...
      MsgBlockState msgSnd = new MsgBlockState
      {
        EntityId = system.Entity.EntityId,
        State = system.State.GetTransmit()
      };
      ...
      if (steamId == 0)
      {
        if (Mod.Log.ShouldLog(Logging.Level.Communication))
        {
          Mod.Log.Write(Logging.Level.Communication, "DrillSystemMod: SyncBlockStateSend to others EntityId={0}/{1}, State={2}", system.Entity.EntityId, Logging.BlockName(system.Entity, Logging.BlockNameOptions.None), msgSnd.State.ToString());
        }
      ...
      else
      {
        if (Mod.Log.ShouldLog(Logging.Level.Communication))
        {
          Mod.Log.Write(Logging.Level.Communication, "DrillSystemMod: SyncBlockStateSend to SteamId={0} EntityId={1}/{2}, State={3}", steamId, system.Entity.EntityId, Logging.BlockName(system.Entity, Logging.BlockNameOptions.None), msgSnd.State.ToString());
        }
      ...
      if (!res && Mod.Log.ShouldLog(Logging.Level.Error))
      {
        Mod.Log.Write(Logging.Level.Error, "DrillSystemMod: SyncBlockStateSend Failed");
      }
    }
    ...
    private void SyncBlockStateReceived(byte[] dataRcv)
    {
      try
      {
        MsgBlockState msgRcv = MyAPIGateway.Utilities.SerializeFromBinary<MsgBlockState>(dataRcv);
        ...
        if (DrillSystems.TryGetValue(msgRcv.EntityId, out system))
        {
          if (Mod.Log.ShouldLog(Logging.Level.Communication))
          {
            Mod.Log.Write(Logging.Level.Communication, "DrillSystemMod: SyncBlockStateReceived EntityId={0}/{1}, State={2}", system.Entity.EntityId, Logging.BlockName(system.Entity, Logging.BlockNameOptions.None), msgRcv.State.ToString());
          }
        ...
        else
        {
          if (Mod.Log.ShouldLog(Logging.Level.Error))
          {
            Mod.Log.Write(Logging.Level.Error, "DrillSystemMod: SyncBlockStateReceived for unknown system EntityId={0}", msgRcv.EntityId);
          }
        }
      }
      catch (Exception ex)
    ...
    public static void SyncBlockVoxelBoxesFillRemoveAddQueue(MyVoxelBase voxelBase, Vector3I voxelCoordMin, Vector3I voxelCoordMax, List<BoundingBoxI> boxes, byte material, long content)
    {
      ...
      {
        SyncEntityId syncId = SyncEntityId.GetSyncId(voxelBase);
        foreach (SyncTargetVoxelBoxesRemoveFillData voxelBoxes in _MsgVoxelBoxesFillRemove.VoxelBoxesData)
        {
        ...
        //New VoxelBoxes
        SyncTargetVoxelBoxesRemoveFillData voxelBoxesNew = new SyncTargetVoxelBoxesRemoveFillData
        {
          Entity = syncId,
          Boxes = new List<BoundingBoxI>()
        };
    ...
    public static void SyncBlockVoxelBoxesFillRemoveSend()
    {
      ...
      lock (_MsgVoxelBoxesFillRemove)
      {
        if (_MsgVoxelBoxesFillRemove.VoxelBoxesData.Count > 0)
        {
          if (Mod.Log.ShouldLog(Logging.Level.Communication))
          {
            Mod.Log.Write(Logging.Level.Communication, "DrillSystemMod: SyncBlockVoxelBoxesFillRemoveSend to others boxes={0}", _MsgVoxelBoxesFillRemove.VoxelBoxesData.Count);
          }
          ...
          if (!res && Mod.Log.ShouldLog(Logging.Level.Error))
          {
            Mod.Log.Write(Logging.Level.Error, "DrillSystemMod: SyncBlockVoxelBoxesFillRemoveSend Failed");
          }
        }
      }
    }
    ...
    /// 
    /// </summary>
    private void SyncBlockVoxelBoxesFillRemoveReceived(byte[] dataRcv)
    {
      try
      {
        if (MyAPIGateway.Session.IsServer)
        ...
        MsgVoxelBoxesFillRemove msgRcv = MyAPIGateway.Utilities.SerializeFromBinary<MsgVoxelBoxesFillRemove>(dataRcv);

        if (Mod.Log.ShouldLog(Logging.Level.Communication))
        {
          Mod.Log.Write(Logging.Level.Communication, "DrillSystemMod: SyncBlockVoxelBoxesFillRemoveReceived Length={0} Count={1}", dataRcv.Length, msgRcv.VoxelBoxesData.Count);
        }

        MyAPIGateway.Parallel.Start(() =>
        {
          foreach (SyncTargetVoxelBoxesRemoveFillData voxelBoxes in msgRcv.VoxelBoxesData)
          {
            try
            {
              MyVoxelBase voxelBase = SyncEntityId.GetItemAs<MyVoxelBase>(voxelBoxes.Entity);
              if (Mod.Log.ShouldLog(Logging.Level.Verbose))
              {
                Mod.Log.Write(Logging.Level.Verbose, "DrillSystemMod: VoxelBoxesFillRemove {0} Count={1}", Logging.BlockName(voxelBase), voxelBoxes.Boxes.Count);
              }
            ...
            catch (Exception ex)
            {
              if (Mod.Log.ShouldLog(Logging.Level.Error))
              {
                Mod.Log.Write(Logging.Level.Error, "DrillSystemMod: VoxelBoxesFillRemove Exception:{0}", ex);
              }
            }
          }
        });
